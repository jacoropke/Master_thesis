...
class DLSolverClass():
	...
	def _setup_train_default(self):
		...
		train.store_pd_conditional = False # store pd conditional states in replay buffer
	...
	def _get_quad(self):
		...
		if train.do_quad: # for quadrature
		...
		# EXTRA INSERTED FOR MC
		else: # for mc
			train.Nquad = quad.shape[2]
			assert quad.shape[0] == self.par.T-1, 'quad must have same length as T'
			assert quad.shape[1] == self.train.N, 'quad must have same length as N'
			assert quad.shape[2] == quad_w.shape[0], 'quad and quad_w must have same length'
			assert quad.shape[3] == self.par.Nshocks, 'quad must have same length as Nshocks'
	...
	def _draw_initial_states(self,N,training=False,shocks=None):
		""" draw initial states """
		...
		initial_states = self.draw_initial_states(N,training=training,shocks=shocks) # len(tuple_initial_states) = Nstates, tuple_initial_states[i].shape = (N,)
	...
	def _draw(self,sim,training=False):
		...
		sim.states[0] = self._draw_initial_states(sim.N,training=training, shocks = sim.shocks[0])
	...
	def prepare_simulate_R(self,torch_rng_state=None):
		...
		if torch_rng_state is None:
			...
			# Create and seed a separate generator
			gen = torch.Generator()
			gen.manual_seed(self.par.seed)
		else:
			# Restore global RNG state
			torch.set_rng_state(torch_rng_state)

			# Create a separate generator and restore its state
			gen = torch.Generator()
			gen.set_state(torch_rng_state)
		self.torch_gen = gen
	...
	def _create_repbuffer(self):
		...
		self.rep_buffer = replay_buffer.ReplayBuffer(
			...
			store_pd_conditional=train.store_pd_conditional,
	...
	def _simulate_training_sample(self,epsilon_sigma,do_exo_actions,device=None):
		...
		if train.new_mc and (train.k % train.update_mc_freq == 0):
			self._get_quad() # new mc-shocks
	...
	def _update_best(self):
		...
		if sim.R.item() > best.R: 
			...
			if train.N_value_NN == None:
				best.value_NN = deepcopy(self.value_NN.state_dict()) if not self.value_NN is None else None
			else:
				best.value_NN = []
				for i in range(train.N_value_NN):
					best.value_NN.append(deepcopy(self.value_NNs[i].state_dict()))
	...
	def _print_progress(self,t0_k,t0_solve):
		...
		# print(f'{k = :5d} of {train.K-1}: sim.R = {self.sim.R:12.8f} [best: {best.R:12.8f}] [{time_k:.1f} secs] [{value_epochs = :3d}] [{policy_epochs = :3d}] [{time_tot:6.2f} mins]',end='')
	...
	def solve(self,do_print=False,do_print_all=False,show_memory=False,postfix=''):
		...
		if not continued_solve:
			...
			self.info['time.EGM_target'] = 0.0
			self.info['time.upper_envelope'] = 0.0
		...
		while True:
			...
			# if train.k > 0:
			#     print(self.info['best'].R, "R in best so far")
		...
		if train.N_value_NN == None:
			...
			counter = 0
			for i in range(train.N_value_NN):
				if best.value_NN[i] == None:
					counter +=1
			if counter == 0:
				for i in range(train.N_value_NN):
					self.value_NNs[i].load_state_dict(best.value_NN[i])
