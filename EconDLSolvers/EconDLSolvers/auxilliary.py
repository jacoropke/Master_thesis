...
def expand_to_quad(x,Nquad):
	...
	if len(x.shape) == 2:
		...
		raise ValueError("Invalid shape for x in expand_to_quad")
	# else:
	# 	T,N,NDC,X = x.shape
	# 	print(x.shape)
	# 	return x.reshape(T,N,NDC,1,X).repeat((1,1,1,Nquad,1))
...
def expand_to_states(x, states):
    """Expand the shape of x to match the leading dimensions of states."""
    leading_dims = states.shape[:-1]  # Get all dimensions except the last
    tile_dims = leading_dims + (1,)  # Add 1 to match the shape of x along the last dimension
    return x.tile(tile_dims)
