...
class ReplayBuffer():
	...
	def __init__(self,
			  ...
			  store_actions=True,store_reward=True,store_pd=True,store_pd_conditional=False,i_t_index=False,NDC=0):
		...
		self.store_pd_conditional = store_pd_conditional
		...
		if self.store_pd_conditional: self.states_pd_conditional = torch.zeros((T,self.buffer_size,Nstates_pd,NDC),dtype=dtype,device=device)
	...
	def add(self,train):
		...
		if self.store_pd_conditional: self.states_pd_conditional[:,i0:iend] = train.states_pd_conditional
	...
	def sample(self,batch_size):
		...
		if self.store_pd_conditional: batch.states_pd_conditional = self.states_pd_conditional[row_indices,col_indices]
