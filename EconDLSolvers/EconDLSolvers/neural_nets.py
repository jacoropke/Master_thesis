...
def signed_log_transform(W):
    return torch.sign(W) * torch.log1p(torch.abs(W))
...
def eval_NN(NN,NNT,x,scale_vec=None,t0=0,t=None):
    ...
    if t is not None:
        ...
        eyeT = eyeT.repeat_interleave(Nx,dim=0)
        eyeT = eyeT.reshape((T,Nx,NNT)) # shape = (T,Nx,NNT)
...
class Policy(nn.Module):
    ...
    def __init__(self,par,train):
        ...
        # self.layers[-1].bias.data.fill_(3.0)  # Initialize bias to ~20
        # self.layers[-1].bias.data.fill_(30.0)  # Initialize bias to ~20
    ...
    def forward(self,state):
        ...
        # scale first input
        state_transformed = state.clone()
        state_transformed[..., 0] = signed_log_transform(state[..., 0])  # Apply only to wealth
        
        # input layer
        # s = self.intermediate_activation(self.layers[0](state))
        s = self.intermediate_activation(self.layers[0](state_transformed))
        ...
        if type(self.policy_activation_final) is not list:
            ...
            # action = self.policy_activation_final(self.layers[-1](s) + 10.0)
            
            # action = self.layers[-1](s) + 10.0
            # action = self.policy_activation_final(self.layers[-1](s) + 20.0)
            # action = self.policy_activation_final(self.layers[-1](s)) * 3.0
            ...
            for i_a in range(self.Nactions): # apply activation to each action
                if self.policy_activation_final[i_a] is None:
                    ...
                    # action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a].view(-1,1) + 0.5)
                    # action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a]-0.5.view(-1,1))
                    # action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a].view(-1,1))
                    # action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a].view(-1,1))
                    action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a].view(-1,1) + 2)
                    # action_i_a = self.policy_activation_final[i_a](s_noact[:,i_a].view(-1,1) + 20)
                if i_a == 0:
                    action = action_i_a
                else:
