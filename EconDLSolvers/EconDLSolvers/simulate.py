...
def simulate(model,ns,eps=None,exo_actions=None):
	...
	with torch.no_grad():
		...
		if par.NDC > 0:
			states_pd_conditional = ns.states_pd_conditional
		outcomes = ns.outcomes
		actions = ns.actions
		shocks = ns.shocks
		reward = ns.reward
		discounted_reward = torch.zeros((ns.N),dtype=dtype,device=device)
		...
		for t in range(par.T):
			...
			if par.NDC == 0:
				...
				terminal_reward = par.beta *  model.terminal_reward_pd(states_pd[t])[...,0]
				discounted_reward += discount_factor[t] * (par.uncon_survival[t] * reward[t] + par.uncon_survival[t] * (1-par.s_p[t]) * terminal_reward)
				if t < par.T-1:	
				...
				states_pd_conditional[t] = model.state_trans_pd(states[t],actions[t],outcomes[t],t=t)
				
				# oo. continuation values
				values_DC = reward[t].clone()
				if t < par.T-1:
					for i_DC in range(par.NDC):
						states_ = states_pd_conditional[t,:,:,i_DC]
						cont_value = model.eval_value_pd(value_NN,states_,t=t)[...,0]
						disc_factor = model.discount_factor(states_,t=t)
						values_DC[:,i_DC] += disc_factor*cont_value
				
				# ooo. discrete choice
				DC[t] = torch.argmax(values_DC,dim=-1)
				for i_DC in range(par.NDC):
					I = DC[t] == i_DC
					states_pd[t][I,:] = states_pd_conditional[t,I,:,i_DC]
					reward[t][~I,i_DC] = 0.0
				...
				if t < par.T-1:
					states[t+1] = model.state_trans(states_pd[t],shocks[t+1],t=t)
					# states[t+1] = model.state_trans(states_pd[t],shocks[t],t=t)
	...
	if par.NDC == 0:
		...
		ns.R = torch.sum(discounted_reward)/ns.N
...
def simulate_DeepSimulate(model,policy_NN,initial_states,shocks):
	...
	terminal_reward = torch.zeros((par.T,N),dtype=dtype,device=device)
	discounted_reward = torch.zeros((N,),dtype=dtype,device=device)
	...
	for t in range(par.T):
		...
		reward = model.reward(states_t,actions_t,outcomes_t,t=t)
		discount_factor[t] = model.discount_factor(states_t,t=t)**t

		# iv. transition
		states_pd_t = model.state_trans_pd(states_t,actions_t,outcomes_t,t=t)
		terminal_reward = par.beta * model.terminal_reward_pd(states_pd_t)[...,0]
		discounted_reward += discount_factor[t].clone() * (par.uncon_survival[t] * reward.clone() + par.uncon_survival[t] * (1-par.s_p[t]) * terminal_reward.clone())
		if t < par.T-1:
			new_states_t[:,:] = model.state_trans(states_pd_t,shocks[t+1],t=t)
			# new_states_t[:,:] = model.state_trans(states_pd_t,shocks[t],t=t)
		# else:
		# 	reward[t] += model.terminal_reward_pd(states_pd_t)[...,0]
			
	# d. compute discounted utility
	# R = torch.sum(discount_factor*reward)/train.N
	R = torch.sum(discounted_reward)/train.N
	loss = -R
